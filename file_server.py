...
# RESTful APIs 简易实现，继承 flask.views.MethodView 然后重写方法：GET POST PUT DELETE 等
class PathView(MethodView):
    ...
    def put(self, p=''):
        if request.cookies.get('auth_cookie') == common_key or auth_key:
            path = os.path.join(root, p)
    ...
    def delete(self, p=''):
        if request.cookies.get('auth_cookie') == auth_key:
            path = os.path.join(root, p)
            dir_path = os.path.dirname(path)
            Path(dir_path).mkdir(parents=True, exist_ok=True)
            filename = secure_filename(os.path.basename(path))
